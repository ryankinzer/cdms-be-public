...
{
    public static class BiomChannelUnitMetricsExtensions
    {
        //Extension method to give ServicesContext this property.
        public static DbSet<BiomChannelUnitMetrics_Header> BiomChannelUnitMetrics_Header(this ServicesContext ctx)
        {
            return ctx.GetDbSet("BiomChannelUnitMetrics_Header").Cast<BiomChannelUnitMetrics_Header>();
        }

        public static DbSet<BiomChannelUnitMetrics_Detail> BiomChannelUnitMetrics_Detail(this ServicesContext ctx)
        {
            return ctx.GetDbSet("BiomChannelUnitMetrics_Detail").Cast<BiomChannelUnitMetrics_Detail>();
        }
    }
}
namespace services.Models.Data
{
    public class BiomChannelUnitMetrics : DatasetData
    {
        ...
        public BiomChannelUnitMetrics(int ActivityId)
        {
            ...
            var headers_q = from h in db.BiomChannelUnitMetrics_Header()
                            where h.ActivityId == ActivityId
                            join h2 in
                                (
                                from hh in db.BiomChannelUnitMetrics_Header()
            ...
            var details_q = from h in db.BiomChannelUnitMetrics_Detail()
                            ...
                            join h2 in
                                (
                                    from hh in db.BiomChannelUnitMetrics_Detail()
                                    where hh.EffDt <= DateTime.Now
                                    where hh.ActivityId == ActivityId
