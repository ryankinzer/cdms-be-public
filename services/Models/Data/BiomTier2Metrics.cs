...
{
    public static class BiomTier2MetricsExtensions
    {
        //Extension method to give ServicesContext this property.
        public static DbSet<BiomTier2Metrics_Header> BiomTier2Metrics_Header(this ServicesContext ctx)
        {
            return ctx.GetDbSet("BiomTier2Metrics_Header").Cast<BiomTier2Metrics_Header>();
        }

        public static DbSet<BiomTier2Metrics_Detail> BiomTier2Metrics_Detail(this ServicesContext ctx)
        {
            return ctx.GetDbSet("BiomTier2Metrics_Detail").Cast<BiomTier2Metrics_Detail>();
        }
    }
}
namespace services.Models.Data
{
    public class BiomTier2Metrics : DatasetData
    {
        private Boolean debugMode = true;
        private static Logger logger = LogManager.GetCurrentClassLogger();

        public Dataset Dataset { get; set; }
        public BiomTier2Metrics_Header Header { get; set; }
        public List<BiomTier2Metrics_Detail> Details { get; set; }

        public BiomTier2Metrics()
        {
            Details = new List<BiomTier2Metrics_Detail>();
        }

        // load an existing one
        public BiomTier2Metrics(int ActivityId)
        {
            ...
            var headers_q = from h in db.BiomTier2Metrics_Header()
                            where h.ActivityId == ActivityId
                            join h2 in
                                (
                                from hh in db.BiomTier2Metrics_Header()
            ...
            var details_q = from h in db.BiomTier2Metrics_Detail()
                            where h.ActivityId == ActivityId
                            where h.RowStatusId == DataDetail.ROWSTATUS_ACTIVE
                            join h2 in
                                (
                                    from hh in db.BiomTier2Metrics_Detail()
                                    where hh.EffDt <= DateTime.Now
                                    where hh.ActivityId == ActivityId
